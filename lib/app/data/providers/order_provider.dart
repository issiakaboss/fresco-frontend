...
Future<dynamic> getActiveOrders() async {
  try {
    final response = await ApiProvider.get(
      apiURL: ApiRoutes.orders.path, // Appelle la fonction index() de ton controlleur Laravel
      auth: true,
    ).catchError(handleError);
    return response;
  } catch (e) {
...
Future<dynamic> updateStatus(int orderId, String status) async {
  try {
    final response = await ApiProvider.put(
      apiURL: "${ApiRoutes.orders.path}/$orderId/status", // Ajuste selon ta route Laravel de updateStatus
      auth: true,
      data: {'status': status},
    ).catchError(handleError);
    return response;
